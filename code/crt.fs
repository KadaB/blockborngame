...
varying vec4 fragColorIn;

//Input uniform values
uniform sampler2D texture0;
//uniform vec4      colDiffuse;

vec2 iResolution = vec2(800.0, 450.0);
...
vec3 Fetch(vec2 pos,vec2 off){
  ...
  return ToLinear(texture2D(texture0/*iChannel0*/,pos.xy,-16.0).rgb);}
...
void main( /*out vec4 fragColor, in vec2 fragCoord*/ ){
 ...
 vec4 outFragColor = fragColorIn;


   // Unmodified.
  //if(fragCoord.x<iResolution.x*0.333){ outFragColor.rgb=Fetch(fragCoord.xy/iResolution.xy+vec2(0.333,0.0),vec2(0.0,0.0));}
  //else{
    ...
    outFragColor.rgb=Tri(pos)*Mask(fragCoord.xy);
...
//}
 ...
 // outFragColor.rgb*=
  ...
  outFragColor.rgb=ToSrgb(outFragColor.rgb);
